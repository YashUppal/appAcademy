...
def cl_to_lr
  ...
  execute(<<-SQL)
    ...
    JOIN
      routes b ON (a.company = b.company AND a.num = b.num)
    WHERE
      a.stop_id = 53 AND b.stop_id = (SELECT id FROM stops WHERE name = 'London Road')
  SQL
end
...
def craiglockhart_to_sighthill 
  ...
  # It's a mess but it's honest work :D
  execute(<<-SQL)
    SELECT DISTINCT
      foo.num,foo.company,foo.transfer,bar.bbn,bar.bbc
    FROM
      (
      SELECT
        a.num,a.company,stopb.name as transfer
      FROM
        routes a
      JOIN
        routes b
    ...
    JOIN
        (SELECT
          bb.num as bbn,bb.company as bbc ,stopaa.name as transfera,stopbb.name as transferb
        FROM
          routes aa
        JOIN
          routes bb
        ...
        JOIN
          stops stopaa
        ON
          aa.stop_id = stopaa.id
        JOIN
          stops stopbb
        ON
          bb.stop_id = stopbb.id
        WHERE
          stopaa.name IN (SELECT
       stopb.name
      FROM
        routes a
      JOIN
      ...
      WHERE
        stopa.name = 'Craiglockhart')
        AND
          stopbb.name = 'Sighthill') AS bar
      ON
          foo.transfer = bar.transfera
