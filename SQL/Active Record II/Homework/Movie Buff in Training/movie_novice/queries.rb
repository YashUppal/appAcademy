...
def below_average_years
  ...
  Movie.select("movies.yr,COUNT(movies.yr) AS bad_movies").where("movies.score < 5").group(:yr).order("bad_movies DESC")
end
...
def pulp_fiction_actors
  ...
  Movie.joins(:actors).select("actors.id,actors.name").where("movies.title = 'Pulp Fiction'")
end 
...
def uma_movies
  ...
  Movie.joins(:castings).select("movies.id,movies.title,movies.yr").where("castings.actor_id = ?",Actor.find_by(name: 'Uma Thurman').id)
end
