...
def harrison_ford
  ...
  # Movie.joins(:castings).where("castings.actor_id = ? AND castings.ord != ?",(Actor.find_by(name: 'Harrison Ford').id),1).select("movies.id,movies.title")
  Movie.joins(:actors).select("movies.id, movies.title").where("actors.name = 'Harrison Ford' AND castings.ord != ?",1)
...
def biggest_cast
  ...
  Movie.joins(:castings).select("movies.id,movies.title").group('movies.id').order("COUNT(movies.id) DESC").limit(3)
end
...
def directed_by_one_of(them)
  ...
  Movie.joins(:director).select("movies.id,movies.title").where("actors.name IN (?)", them)
