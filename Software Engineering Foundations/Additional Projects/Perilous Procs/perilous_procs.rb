...
def filter_out(arr, &prc)
  filtered_array = []
  arr.each do |ele|
    filtered_array << ele if !prc.call(ele)
  end
  filtered_array
end
...
def xnor_select(arr, prc1, prc2)
  xnor_array = []
  arr.each do |ele|
    xnor_array << ele if (prc1.call(ele) && prc2.call(ele)) || (!prc1.call(ele) && !prc2.call(ele))
  end
  xnor_array
end
...
def multi_map(arr, n=1, &prc)
  ...
  arr.each do |ele|
    mapped_ele = ele
    n.times do 
      mapped_ele = prc.call(mapped_ele)
    end
    mapped << mapped_ele
  end
  mapped
...
def selected_map!(arr, prc1, prc2)
  iterator = 0
  while iterator < arr.length
    arr[iterator] = prc2.call(arr[iterator]) if prc1.call(arr[iterator])
    iterator += 1
  end
end
...
def proc_suffix(sentence, proc_hash)
  suffixed = []

  sentence.split.each do |word|
    suffix = ""
    proc_hash.each do |key, value|
      suffix += value if key.call(word)
    end
...
def proctition_platinum(arr, *procs)
  new_array = Array.new(arr)
  proc_hash = Hash.new { |hash,key| hash[key] = Array.new }

  iterator = 0
  while iterator < procs.length
    proc_hash[iterator+1]
    arr.each_with_index do |ele, idx|
      if procs[iterator].call(ele)
        proc_hash[iterator+1] << ele if !proc_hash.values.flatten.include?(ele)
      end
    end
    iterator += 1
  end
...
def procipher(sent, hash)
  ...
  sent.split.each do |word|
    ...
    hash.each do |key, value|
      if key.call(word)
        cipherd = value.call(cipherd)
      end
    end
    procipherd_sent << cipherd
  end
  procipherd_sent.join(" ")
end
